...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.jobGrade.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.job-grades.update", [$jobGrade->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="job_grade_name">{{ trans('cruds.jobGrade.fields.job_grade_name') }}</label>
                <input class="form-control {{ $errors->has('job_grade_name') ? 'is-invalid' : '' }}" type="text" name="job_grade_name" id="job_grade_name" value="{{ old('job_grade_name', $jobGrade->job_grade_name) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\JobGrade::ORGANIZATIONAL_AREA_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('organizational_area') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="organizational_area_{{ $key }}" name="organizational_area" value="{{ $key }}" {{ old('organizational_area', $jobGrade->organizational_area) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('job_grade') ? 'is-invalid' : '' }}" name="job_grade" id="job_grade">
                    ...
                    @foreach(App\JobGrade::JOB_GRADE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('job_grade', $jobGrade->job_grade) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('job_level') ? 'is-invalid' : '' }}" name="job_level" id="job_level" required>
                    ...
                    @foreach(App\JobGrade::JOB_LEVEL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('job_level', $jobGrade->job_level) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
