    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.languages.update", [$language->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('resource_code') ? 'is-invalid' : '' }}" name="resource_code_id" id="resource_code_id" required>
                    @foreach($resource_codes as $id => $resource_code)
                        <option value="{{ $id }}" {{ ($language->resource_code ? $language->resource_code->id : old('resource_code_id')) == $id ? 'selected' : '' }}>{{ $resource_code }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('language_name') ? 'is-invalid' : '' }}" name="language_name" id="language_name" required>
                    ...
                    @foreach(App\Language::LANGUAGE_NAME_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('language_name', $language->language_name) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fluency_level') ? 'is-invalid' : '' }}" type="text" name="fluency_level" id="fluency_level" value="{{ old('fluency_level', $language->fluency_level) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('certificate_level') ? 'is-invalid' : '' }}" type="text" name="certificate_level" id="certificate_level" value="{{ old('certificate_level', $language->certificate_level) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('year_level') ? 'is-invalid' : '' }}" type="text" name="year_level" id="year_level" value="{{ old('year_level', $language->year_level) }}">
