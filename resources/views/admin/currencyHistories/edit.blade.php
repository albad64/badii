...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.currencyHistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.currency-histories.update", [$currencyHistory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('currency') ? 'is-invalid' : '' }}" name="currency_id" id="currency_id" required>
                    @foreach($currencies as $id => $currency)
                        <option value="{{ $id }}" {{ ($currencyHistory->currency ? $currencyHistory->currency->id : old('currency_id')) == $id ? 'selected' : '' }}>{{ $currency }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_validity') ? 'is-invalid' : '' }}" type="text" name="date_validity" id="date_validity" value="{{ old('date_validity', $currencyHistory->date_validity) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('conversion_rate') ? 'is-invalid' : '' }}" type="number" name="conversion_rate" id="conversion_rate" value="{{ old('conversion_rate', $currencyHistory->conversion_rate) }}" step="0.000001">
